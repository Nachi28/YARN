...
function change(x) {
    ...
    //Flip
        ...
        if (rem == 0) {
            ...
            if (min == 0) {
                time = `${sec} seconds`;
                gameOver(sec); // Call the gameOver() function

            }
            else {
                time = `${min} minute(s) and ${sec} second(s)`;
                gameOver(sec + min * 60); // Call the gameOver() function
...
function gameOver(sec) {
    clearInterval(time); // Stop the timer
    const gameOverData = {
        mode: mode, // Game mode (e.g., "3x4")
        moves: moves, // Number of moves
        time: sec // Time taken to complete the game
    };
    console.log(gameOverData);

    // Send a POST request to the server with the game over data
    fetch("/memoryCard-game-over", {
        method: "POST",
        headers: {
